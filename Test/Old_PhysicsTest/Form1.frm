...
Private Sub Form_Load()
    ...
    Call gfx.Initialize(Me.hWnd, 800, 600, 32, False, True)
    ...
    Dim t As physics.Body
    
    Set t = sim.Bodies.Add(326, 490 - 100, 0, 551, 24, 0, 0, True, 3)
    Set t = sim.Bodies.Add(608, 333 - 100, 0, 12, 800, 0, 0, True, 2)
    Set t = sim.Bodies.Add(57, 628 - 100, 0, 12, 300, 0, 0, True, 2)
    
    
    ' Escalera:
    Dim s As Integer
    For s = 0 To 4
        Set t = sim.Bodies.Add(500 + (24 * s) - 6, 478 - (12 * s) - 100, 0, 24, 24, 0, 0, True, 3)
    Next
    ...
    ' Entidad no fisica:
    Set t = sim.Bodies.Add(250, 325, 0, 200, 100, 0, 0, False, 99)
    t.PhysicType = NoPhysicalEntity
    t.Color = &HFFFFFF00
    t.Tag = "Trigger"
    
    ' Entidad fisica:
    Set a = sim.Bodies.Add(100, 100, 0, 24, 100, 1, 3.5, False, 18)
    a.Color = &HFFFF7700
    a.PhysicType = PhysicalEntity
    a.Tag = "Player"
    
    ' Entidad fisica:
    Set b = sim.Bodies.Add(212, 300, 1, 24, 100, 1, 1, False, 18)
    b.Color = &HFF0000FF
    b.PhysicType = PhysicalEntity
    b.Tag = "Friend"

    ' Cuerpo normal (no entidad):
    Set C = sim.Bodies.Add(250, 100, 0, 100, 100, 1, 2, False, 18)

    ' Entidad fisica (enemigo):
    Set D = sim.Bodies.Add(312, 300, 2, 24, 100, 1, 1, False, 18)
    D.Color = &HFFFF0000
    D.PhysicType = PhysicalEntity
    D.Tag = "Enemy"
    
    Dim pt As System.Point, source As System.Point, target As System.Point
...
'If sys.TIMER_GetValue(tim) >= 25 Then
        ...
        Dim angle As Single, mouse As System.Point
        mouse.X = gameInput.mouse.X: mouse.Y = gameInput.mouse.Y
        angle = System.Math.GetAngle(source, mouse)

        'If sim.TraceRay(source, target, pt) Then
        If Not a.TraceRay(source, angle, pt, target, physics.Helper.CreateZDiscardArray(1, 2)) Is Nothing Then
